...
class ProductRepo @Inject()(db: Database) extends ProductRepoLike {
  ...
  def addProduct(name: String, description: String): Future[Product] = {
    val action = Tables.Products.map(p => (p.name, p.description)) += ((name, description))
